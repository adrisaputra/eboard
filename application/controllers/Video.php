...
class Video extends Admin_Controller {
	...
	public function create()
	{
		...
		if (empty($_FILES['userfile']['name']))
			{	
				$this->form_validation->set_rules('userfile', 'File Video', 'required');
			}
		...
		{	
			...
			$config['allowed_types']    = 'mp4|ogg|webm|flv|mkv|mpg';
	...
	public function update()
	{
		...
		{	
			...
			if (!$this->upload->do_upload('userfile')) {
				...
				$entry =  $this->m_video->get($this->input->post('video_id'));
				$path_file = './upload/video/';
				unlink($path_file.$entry[0]['video_file']);
	...
	public function delete() 
	{
		...
		$video_id = decrypt_url($this->uri->segment(3));
		$entry = $this->m_video->get($video_id);
